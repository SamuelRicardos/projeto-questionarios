  ...
  return (
    ...
    >
      <div className="max-w-[1500px] mx-auto flex justify-between items-center p-1">
        ...
        <nav
          className={`hidden md:flex gap-6 pr-4 ${theme === "dark" ? "text-gray-200" : "text-gray-800"
            }`}
        >
          <button
            onClick={toggleTheme}
        ...
        <div className="md:hidden flex gap-8">
          <button
            onClick={toggleTheme}
            className="md:hidden focus:outline-none hover:text-gray-400 transition cursor-pointer"
          >
            {theme === "light" ? (
              <FaMoon className="text-gray-800" size={20} />
            ) : (
          ...
          <div
            className={`md:hidden cursor-pointer ${theme === "dark" ? "text-gray-200" : "text-gray-800"
              }`}
            onClick={toggleMenu}
          >
      ...
      {isOpen && (
        <div
          className={`md:hidden flex flex-col items-center gap-4 py-4 ${theme === "dark" ? "bg-gray-800 text-gray-200" : "bg-[#faf7ed]"
            }`}
        >
          <Link
            to="/login"
